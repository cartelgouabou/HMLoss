...
# normMean,normStd = compute_img_mean_std(np.array(df_train.path))
# print('normmean:')
# print(normMean)
# print('norstd:')
# print(normStd)
...
if args.loss_type == 'CE':
    ...
    criterion_hm = SoftmaxHardMiningLoss(weight=per_cls_weights,delta=args.delta).to(device)
elif args.loss_type == 'Focal':
    criterion = FocalLoss(weight=per_cls_weights).to(device)
elif args.loss_type == 'LDAM':
    criterion = LDAMLoss(weight=per_cls_weights,cls_num_list=cls_num_list).to(device)
else:
...
validation_set = CustomImageDataset(df_val, transform=train_transform)
...
if 'HML' in args.loss_type:
    args.store_name = '_'.join(['isic2019', 'efficientb3', args.loss_type, 'W' ,args.weighting_type,'F',str(args.ratio_finetune),'D',str(args.delta)])
else:
    args.store_name = '_'.join(['isic2019', 'efficientb3', args.loss_type, 'W' ,args.weighting_type,'F',str(args.ratio_finetune)])
...
for run in run_list:
    ...
    for epoch in tqdm(range(1, epoch_num+1)):
        loss_train, acc_train, total_loss_train, total_acc_train = train(train_loader, model, criterion, criterion_hm,optimizer,args, epoch, device,hml)
        loss_val, acc_val = validate(val_loader, model, criterion, criterion_hm, optimizer,args, epoch, device,hml)

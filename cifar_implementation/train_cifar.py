...
parser.add_argument('--imb_ratio_list','--ir_list', nargs='+',metavar='N', type=float, default=1, help='imbalance ratio, 1 for balanced, 0.1 or 0.01; use case: --ir 1 0.1 0.01',dest='ir_list')
...
parser.add_argument('--pursue', default=False, help='continue a previous work',dest='pursue')
parser.add_argument('--run_to_pursue', default='run0', type=str, help='run to pursue the previous work',dest='run_to_pursue')
...
print(args.ir_list)
print('Continue a previous work?:')
print(args.pursue)


if args.pursue==False:
    for run in run_list:
        for ir in args.ir_list:
            imbalanceData = IMBALANCECIFAR(dataset_name=args.dataset_name,  imb_type=args.imb_type, imb_factor=ir)
            ...
            filenames=args.dataset_name+'_'+args.imb_type+'_'+args.case_loss_fn+'_ir_'+str(ir)+'_'+run
            ...
            title_text=args.dataset_name+'_'+args.imb_type+'_'+args.case_loss_fn+'_ir_'+str(ir)+'_'+run
            learning_curve(resultat,checkpoint_root+'/training_history',title_text)
    
else:
    run_list=run_list[run_list.index(args.run_to_pursue):len(run_list)]
    for run in run_list:
        for ir in args.ir_list:
            imbalanceData = IMBALANCECIFAR(dataset_name=args.dataset_name,  imb_type=args.imb_type, imb_factor=ir)
            x_train=imbalanceData.train_images
